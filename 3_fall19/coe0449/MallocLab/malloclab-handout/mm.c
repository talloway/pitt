...
static BlockInfo* searchFreeList(size_t reqSize) {
  ...
  freeBlock = FREE_LIST_HEAD;
	...
	while (freeBlock != NULL) {
		tempSize = freeBlock->sizeAndTags;
		if (tempSize >= reqSize)
			return freeBlock;
...
void* mm_malloc(size_t size) {
  ...
  freeBlock = searchFreeList(reqSize);
	...
	// Should only need to do it once more
	if (freeBlock == NULL) {
		requestMoreSpace(reqSize);
		freeBlock = searchFreeList(reqSize);
	}
	
	prevTag     = freeBlock->sizeAndTags & TAG_PRECEDING_USED;
	blockSize   = SIZE(freeBlock->sizeAndTags);	
	payloadSize = blockSize - WORD_SIZE;
	removeFreeBlock(freeBlock);
	
	footer = (int *) UNSCALED_POINTER_ADD(freeBlock, payloadSize);
	
	if ((blockSize - reqSize) < MIN_BLOCK_SIZE) {
    followingBlock = (BlockInfo *) UNSCALED_POINTER_ADD(freeBlock, blockSize);
		followingBlock->sizeAndTags = followingBlock->sizeAndTags | TAG_PRECEDING_USED;

		*footer = blockSize;
	}

	// Split blocks if big enough
	else {
		freeBlock->sizeAndTags = reqSize | prevTag;

		followingBlock = (BlockInfo *) UNSCALED_POINTER_ADD(freeBlock, reqSize);
		followingBlock->sizeAndTags = (((blockSize - reqSize) | TAG_PRECEDING_USED) & ~TAG_USED);

		*footer = blockSize - reqSize;
		insertFreeBlock(followingBlock);
	}

	freeBlock->sizeAndTags = freeBlock->sizeAndTags | TAG_USED;
	return ((void *) UNSCALED_POINTER_ADD(freeBlock, WORD_SIZE));
}
...
void mm_free(void *ptr) {
	...
	blockHeader = (BlockInfo *) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

	payloadSize = SIZE(blockHeader->sizeAndTags);
	blockHeader->sizeAndTags = blockHeader->sizeAndTags & ~TAG_USED;
...
// To implement:
// 1. Checking allocate/free using bit masks
int mm_check() {
	printFreeList();
  return;
}
...
void printFreeList() {
	...
	printf("Free list head is %p.\n", FREE_LIST_HEAD); // Kinda useless but decent for keeping track of things
	int blockIndex = 0;
	BlockInfo * blockCursor = FREE_LIST_HEAD;
	while (blockCursor != NULL) {
		printf("Address of block %d is %p\n", blockIndex++, blockCursor);
