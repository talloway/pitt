...
__global__ void mm_gpu(float* C, float* A, float* B, int n)
{
	...
	C[i*n + j] = Cvalue;
	// printf("blockIdx.x: %d. blockIdx.y: %d. threadIdx.x: %d. threadIdx.y: %d. result = %d\n", blockIdx.x, blockIdx.y, threadIdx.x, threadIdx.y, C[i*n + j]);
...
__global__ void mm_gpu_shared(float* C, float* A, float* B, int n)
{
	...
	for (int t = 0; t < (n / block_size); t++) {
		As[threadIdx.y*block_size + threadIdx.x] = A[row*n + t*block_size + threadIdx.x];
		Bs[threadIdx.y*block_size + threadIdx.x] = B[(t*block_size + threadIdx.y)*n + col];
