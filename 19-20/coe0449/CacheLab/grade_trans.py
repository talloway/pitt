...
#    print "%-22s%8s%10s%12s" % ("", "Points", "Max pts", "Misses")
    ...
    test_results.append({"score":trans32_score, "max_score":maxscore['trans32'], "output":'Trans Perf. 32x32 - Cache Misses: %s' % (misses)})
    ...
    test_results.append({"score":trans64_score, "max_score":maxscore['trans64'], "output":'Trans Perf. 64x64 - Cache Misses: %s' % (misses)})
    ...
    print json.dumps(output_results, ensure_ascii=False)
