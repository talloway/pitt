 ...
 public int countLevels()
  ...
  private void calcLevelCounts( BSTNode root, int levelCounts[], int level )
  {
	...
	private void addToArrayList(ArrayList<T> keys, BSTNode<T> root)
	{
	 ...
	 private void addKeysInBalancedOrder ( ArrayList<T> keys, int lo, int hi, BSTreeL8<T> balancedBST )  // V L R
	 {
		 if (hi < lo) return;
	...
	private boolean equals( BSTNode<T> thisRoot, BSTNode<T> otherRoot )
	{
		...
		if ((thisRoot == null && otherRoot != null) || (otherRoot == null && thisRoot != null)) return false;
		if (!Objects.equals(thisRoot.key, otherRoot.key)) return false;
